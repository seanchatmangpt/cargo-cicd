...
mod tests {
    ...
    /// sarif_output_has_correct_version — the serialized report must carry
    /// version == "2.1.0" at the top level.
    #[test]
    fn sarif_output_has_correct_version() {
        let report = evidence_issues_to_sarif(&[], "2026-06-28T00:00:00Z", "2026-06-28T00:01:00Z", 1);
        let json = serde_json::to_string(&report).expect("serialize must succeed");
        let parsed: serde_json::Value = serde_json::from_str(&json).expect("parse must succeed");
        assert_eq!(parsed["version"], "2.1.0", "SARIF version must be 2.1.0");
    }

    /// sarif_report_has_schema_field — the $schema field must be present and
    /// reference the SARIF 2.1.0 schema URL.
    #[test]
    fn sarif_report_has_schema_field() {
        let report = evidence_issues_to_sarif(&[], "2026-06-28T00:00:00Z", "2026-06-28T00:01:00Z", 1);
        assert!(
            report.schema.contains("sarif-schema-2.1.0"),
            "$schema must reference the SARIF 2.1.0 schema"
        );
    }

    /// sarif_error_issue_maps_to_cicd_e1 — an error-level EvidenceIssue must
    /// produce a result with ruleId CICD-E1 and level "error".
    #[test]
    fn sarif_error_issue_maps_to_cicd_e1() {
        let issue = EvidenceIssue::error("evidence file missing", "2026-06-28T00:00:00Z");
        let report = evidence_issues_to_sarif(
            &[issue],
        ...
        assert_eq!(result.rule_id, "CICD-E1");
        assert_eq!(result.level, "error");
    }

    /// sarif_execution_successful_false_when_errors_present — invocation must
    /// report executionSuccessful=false when at least one error is present.
    #[test]
    fn sarif_execution_successful_false_when_errors_present() {
        let issue = EvidenceIssue::error("something failed", "2026-06-28T00:00:00Z");
        let report = evidence_issues_to_sarif(
            &[issue],
            "2026-06-28T00:00:00Z",
            "2026-06-28T00:01:00Z",
        ...
        assert!(
            !report.runs[0].invocations[0].execution_successful,
            "executionSuccessful must be false when errors are present"
        );
    }

    /// sarif_execution_successful_true_with_no_issues — invocation must report
    /// executionSuccessful=true when no issues are present.
    #[test]
    fn sarif_execution_successful_true_with_no_issues() {
        let report = evidence_issues_to_sarif(&[], "2026-06-28T00:00:00Z", "2026-06-28T00:01:00Z", 1);
        assert!(
            report.runs[0].invocations[0].execution_successful,
            "executionSuccessful must be true when no issues are present"
        );
    }
}

...
mod tests {
    ...
    fn is_certified_empty_registry_returns_false() {
        assert!(!is_certified(&[], "serde", "1.0.0"));
    }

    #[test]
    fn is_certified_found() {
        ...
        assert!(is_certified(&[entry], "serde", "1.0.0"));
    }

    #[test]
    fn is_certified_version_mismatch() {
        let entry = SafetyCriticalEntry::new(
            "serde",
            "1.0.0",
            "ferrous-systems",
        ...
        assert!(!is_certified(&[entry], "serde", "2.0.0"));
    }

    #[test]
    fn format_registry_listing_empty_has_header() {
        let listing = format_registry_listing(&[]);
        assert!(
            listing.contains("Safety-Critical Crate Registry"),
            "listing header missing"
        );
    }

    #[test]
    fn format_registry_listing_shows_crate_name() {
        let entry = SafetyCriticalEntry::new(
            "my-safe-crate",
            "0.2.0",
            "trail-of-bits",
            vec!["Custom Security Audit".to_string()],
            "2026-06-17",
            "sha256:deadbeef",
        );
        let listing = format_registry_listing(&[entry]);
        assert!(listing.contains("my-safe-crate"));
        assert!(listing.contains("trail-of-bits"));
    }

    #[test]
    fn entry_can_be_constructed() {
        let entry = SafetyCriticalEntry::new(
            "example-crate",
            "1.2.3",
            "trustinsoft",
    ...
    #[test]
    fn parse_simple_toml_entry() {
        let toml = r#"
[[entries]]
crate_name = "safe-lib"
version = "0.1.0"
cert_body_id = "ferrous-systems"
standards = ["IEC 61508 SIL 2"]
certified_at = "2026-06-17"
receipt_hash = "sha256:abc"
"#;
        let entries = parse_toml_entries(toml);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].crate_name, "safe-lib");
        assert_eq!(entries[0].standards, vec!["IEC 61508 SIL 2"]);
    }
}

...
mod tests {
    ...
    #[test]
    fn criteria_returns_at_least_six() {
        assert!(criteria().len() >= 6);
    }

    #[test]
    fn all_criteria_satisfied_by_default() {
        let all = criteria();
        let unsatisfied: Vec<_> = all.iter().filter(|c| !c.satisfied).collect();
        assert!(
            unsatisfied.is_empty(),
            "expected all default criteria to be satisfied, unsatisfied: {:?}",
            unsatisfied.iter().map(|c| &c.id).collect::<Vec<_>>()
        );
    }

    #[test]
    fn compliance_summary_contains_soc2_header() {
        let summary = compliance_summary();
        assert!(
            summary.contains("SOC2"),
            "expected 'SOC2' in summary, got:\n{}",
            summary
        );
    }

    #[test]
    fn compliance_summary_shows_100_percent_when_all_satisfied() {
        let summary = compliance_summary();
        assert!(
            summary.contains("100%"),
            "expected 100% coverage when all criteria satisfied, got:\n{}",
            summary
        );
    }

    #[test]
    fn trust_category_names_are_non_empty() {
        let categories = [
            TrustCategory::Security,
            TrustCategory::Availability,
            TrustCategory::ProcessingIntegrity,
            TrustCategory::Confidentiality,
            TrustCategory::Privacy,
        ];
    ...
    #[test]
    fn criteria_have_non_empty_ids_and_commands() {
        for c in criteria() {
            assert!(!c.id.is_empty(), "criterion id must not be empty");
            assert!(
                !c.covered_by_commands.is_empty(),
                "criterion {} must list at least one command",
                c.id
            );
        }
    }

...
mod tests {
    ...
    #[test]
    fn sil_new_clamps() {
        assert_eq!(Sil::new(0).level(), 1);
        assert_eq!(Sil::new(5).level(), 4);
    }

    #[test]
    fn sil_name_coverage() {
        assert_eq!(Sil::new(1).name(), "SIL 1");
        assert_eq!(Sil::new(2).name(), "SIL 2");
        assert_eq!(Sil::new(3).name(), "SIL 3");
        assert_eq!(Sil::new(4).name(), "SIL 4");
    }

    #[test]
    fn requirements_returns_at_least_eight() {
    ...
    fn check_requirement_satisfied_when_command_present() {
        ...
        let req = reqs.iter().find(|r| r.number == "7.4.5").unwrap();
    ...
    fn check_requirement_missing_when_no_commands() {
        ...
        assert!(gap.is_some(), "should report gap when no commands");
    }
}

...
mod tests {
    ...
    fn ocel_replay_broken_chain_detected() {
        ...
        let ocel_dir = std::path::Path::new(repo_dir)
            .join(".cargo-cicd")
            .join("ocel");
        ...
        assert!(matches!(
            summary.status,
            ReplayStatus::ChainBroken { index: 2 }
        ));
    }
}
...
pub fn append_ocel_event(
    repo_dir: &str,
    event_type: &str,
    objects: serde_json::Value,
    git_delta: &str,
) -> Result<OcelEventRecord, String> {
...
pub fn replay_events(events_jsonl_path: &std::path::Path) -> Result<ReplaySummary, String> {
    ...
    for (i, line) in reader.lines().enumerate() {
        ...
        if computed_hash != record.event_hash {
            return Ok(ReplaySummary {
                events_verified: i,
                status: ReplayStatus::HashInvalid { index: i },
            });
        }
        ...
        if record.prev_hash != expected_prev_hash {
            return Ok(ReplaySummary {
                events_verified: i,
                status: ReplayStatus::ChainBroken { index: i },
            });
        }

...
impl NounCommand for HooksNoun {
    ...
    fn verbs(&self) -> Vec<Box<dyn VerbCommand>> {
        vec![
            Box::new(HooksInstallVerb),
            Box::new(HooksUninstallVerb),
        ]
    }
}
...
impl VerbCommand for HooksInstallVerb {
    ...
    fn run(&self, _args: &VerbArgs) -> clap_noun_verb::error::Result<()> {
        ...
        println!("{}", theme::paint("▶ Installing cargo-cicd hooks\n", Role::Info));

        let repo_root_cmd = std::process::Command::new("git")
            .args(["rev-parse", "--show-toplevel"])
            .output()
            .map_err(|e| clap_noun_verb::error::NounVerbError::execution_error(e.to_string()))?;
        let repo_root_str = String::from_utf8_lossy(&repo_root_cmd.stdout).trim().to_string();
        if repo_root_str.is_empty() {
            return Err(clap_noun_verb::error::NounVerbError::execution_error("Not a git repository. Run this command from a git root."));
        }
        ...
        println!("{}", theme::paint("▶ Installing .git/hooks/pre-commit", Role::Info));
        if !hooks_dir.exists() {
            if let Err(e) = fs::create_dir_all(&hooks_dir) {
                return Err(clap_noun_verb::error::NounVerbError::execution_error(format!("Failed to create hooks directory: {}", e)));
            }
        ...
        if src_hook.exists() {
            if let Err(e) = fs::copy(&src_hook, &dst_hook) {
                return Err(clap_noun_verb::error::NounVerbError::execution_error(format!("Failed to copy hook: {}", e)));
            }
            if let Ok(metadata) = fs::metadata(&dst_hook) {
                ...
                println!("{}", theme::paint("✓ Installed .git/hooks/pre-commit (executable)", Role::Success));
            }
        } else {
            eprintln!("✗ Hook file not found at {}", src_hook.display());
            return Err(clap_noun_verb::error::NounVerbError::execution_error("Hook file not found"));
        }

        // 2. Install pre-commit framework config (optional logic from bash script)
        println!("{}", theme::paint("▶ Setting up pre-commit framework", Role::Info));
        ...
        if std::process::Command::new("pre-commit").arg("--version").output().is_ok() {
            ...
            if std::io::stdin().read_line(&mut input).is_ok() && input.trim() == "y" {
                if std::process::Command::new("pre-commit").arg("install").status().is_ok() {
                    println!("{}", theme::paint("✓ Pre-commit environments installed", Role::Success));
                    let _ = std::process::Command::new("pre-commit").arg("run").arg("--all-files").status();
                }
            }
        } else {
        ...
        println!("{}", theme::paint("▶ Setting up forbidden-terms checker", Role::Info));
...
"#;
        ...
        if fs::write(&checker_path, checker_script).is_ok() {
            if let Ok(metadata) = fs::metadata(&checker_path) {
                ...
                println!("{}", theme::paint("✓ Created scripts/check-forbidden-terms.sh", Role::Success));
            }
        }
        ...
        if std::process::Command::new("bash").arg(dst_hook.to_str().unwrap()).arg("/dev/null").output().is_ok() {
            println!("{}", theme::paint("✓ Hook executable and functional", Role::Success));
        } else {
        ...
        println!("{}", theme::paint("✓ Ready to enforce code quality!", Role::Success));
        Ok(())
    }
}
...
impl VerbCommand for HooksUninstallVerb {
    ...
    fn run(&self, _args: &VerbArgs) -> clap_noun_verb::error::Result<()> {
        ...
        let repo_root_str = String::from_utf8_lossy(&repo_root_cmd.stdout).trim().to_string();
        if repo_root_str.is_empty() {
            return Err(clap_noun_verb::error::NounVerbError::execution_error("Not a git repository."));
        }
        ...
        if pre_commit_hook.exists() {
            ...
            println!("{}", theme::paint("✓ Removed .git/hooks/pre-commit", Role::Success));
        }
        ...
        if pre_commit_yaml.exists() {
            ...
            if std::io::stdin().read_line(&mut input).is_ok() && input.trim() == "y" {
                ...
                println!("{}", theme::paint("✓ Removed .pre-commit-config.yaml", Role::Success));
            }
        }
        ...
        if std::process::Command::new("pre-commit").arg("--version").output().is_ok() {
            ...
            let mut input = String::new();
            if std::io::stdin().read_line(&mut input).is_ok() && input.trim() == "y" {
                if std::process::Command::new("pre-commit").arg("uninstall").status().is_ok() {
                    println!("{}", theme::paint("✓ Pre-commit framework uninstalled", Role::Success));
                }
            }
        }

        println!("\n{}", theme::paint("✓ Hooks uninstalled. You can commit without restrictions.", Role::Success));
        Ok(())
    }
}

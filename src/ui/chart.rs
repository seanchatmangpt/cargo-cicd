...
mod tests {
    ...
    #[test]
    fn sparkline_one_cell_per_sample() {
        let out = sparkline(&[1.0, 2.0, 3.0]);
    ...
    #[test]
    fn meter_is_exact_width() {
        for w in [1usize, 4, 10, 23] {
            for f in [0.0, 0.13, 0.5, 0.87, 1.0] {
                assert_eq!(
                    display_width(&meter(f, w)),
                    w,
                    "meter({f}, {w}) must be {w} columns"
                );
            }
        }
    }
    ...
    #[test]
    fn gauge_shows_value_and_ratio() {
        let out = gauge(5.0, 10.0, 10);
        let plain = text::strip_ansi(&out);
        assert!(plain.contains('5'), "gauge should print the value: {plain}");
        assert!(
            plain.contains('%') || plain.contains('/'),
            "gauge should print a percent or ratio: {plain}"
        );
    }

    #[test]
    fn gauge_clamps_overflow() {
        // value > max must not panic or exceed 100%.
        let plain = text::strip_ansi(&gauge(50.0, 10.0, 8));
        assert!(plain.contains("100%"), "{plain}");
    }

    #[test]
    fn barchart_aligns_rows() {
        let out = barchart(&[("alpha", 3.0), ("b", 9.0)], 12);
        let lines: Vec<&str> = out.lines().collect();
        assert_eq!(lines.len(), 2);
        // Both rows have identical display width once labels/values are padded.
        assert_eq!(display_width(lines[0]), display_width(lines[1]));
    }

    #[test]
    fn histogram_counts_into_buckets() {
        let data = [0.0, 0.1, 0.9, 1.0, 0.5];
        let out = histogram(&data, 2, 10);
        assert_eq!(out.lines().count(), 2);
        assert_eq!(histogram(&[], 4, 10), "");
        assert_eq!(histogram(&data, 0, 10), "");
    }

    #[test]
    fn trend_has_sparkline_and_delta() {
        let plain = text::strip_ansi(&trend(&[1.0, 2.0, 4.0]));
        // Up trend: delta is +3.00.
        assert!(plain.contains("+3.00"), "{plain}");
        let down = text::strip_ansi(&trend(&[4.0, 1.0]));
        assert!(down.contains("-3.00"), "{down}");
        assert_eq!(trend(&[]), "");
    }

    #[test]
    fn forced_color_meter_has_ansi() {
        caps::set_color_override(Some(true));
        let out = meter(0.5, 10);
        assert!(
            out.contains('\u{1b}'),
            "forced-color meter must contain ANSI escapes"
        );
        // And it still measures the right visible width.
        assert_eq!(display_width(&out), 10);
        caps::set_color_override(None);
    }

    #[test]
    fn forced_color_sparkline_has_ansi() {
        caps::set_color_override(Some(true));
        let out = sparkline(&[1.0, 5.0, 2.0, 8.0]);
    ...
    #[test]
    fn plain_mode_is_uncolored() {
        caps::set_color_override(Some(false));
        let out = meter(0.5, 10);
        assert!(!out.contains('\u{1b}'), "plain meter must have no ANSI");
        assert_eq!(display_width(&out), 10);
        caps::set_color_override(None);
    }
}

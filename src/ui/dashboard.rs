...
mod tests {
    ...
    fn populated() -> DashboardData {
        DashboardData {
            toolchain: "stable-x86_64".to_string(),
            branch: "feature/ui-dashboard".to_string(),
            target_gb: 3.4,
            target_cap_gb: 8.0,
            dirty_files: 2,
            untracked: 1,
    ...
    #[test]
    fn default_render_is_non_empty_and_branded() {
        let out = render(&DashboardData::default());
        assert!(!out.is_empty(), "default render must not be empty");
        assert!(
            out.contains("cargo-cicd"),
            "default render must contain the brand substring"
        );
    }

    #[test]
    fn default_render_never_panics_on_empty_collections() {
        // Empty history and policies must be tolerated without panicking.
        let data = DashboardData::default();
        let out = render(&data);
        // The placeholder policy text is present when there are none.
        assert!(strip_ansi(&out).contains("no policy results"));
    }

    #[test]
    fn populated_render_contains_branch() {
        let data = populated();
        let out = render(&data);
        assert!(
            strip_ansi(&out).contains("feature/ui-dashboard"),
            "populated render must contain the branch string"
        );
    }

    #[test]
    fn populated_render_contains_brand_and_toolchain() {
        let data = populated();
        let out = strip_ansi(&render(&data));
        assert!(out.contains("cargo-cicd"));
        assert!(out.contains("stable-x86_64"));
    }

    #[test]
    fn plain_mode_output_has_no_ansi() {
        caps::set_color_override(Some(false));
        caps::set_unicode_override(Some(false));
        let out = render(&populated());
        assert!(
            !out.contains('\u{1b}'),
            "plain mode output must not contain ANSI escapes"
        );
        assert!(out.contains("cargo-cicd"));
    ...
    #[test]
    fn forced_color_output_has_ansi() {
        caps::set_color_override(Some(true));
        let out = render(&populated());
        assert!(
            out.contains('\u{1b}'),
            "forced-color output should carry ANSI escapes"
        );
        // Brand and branch survive ANSI stripping.
        let plain = strip_ansi(&out);
        assert!(plain.contains("cargo-cicd"));
        assert!(plain.contains("feature/ui-dashboard"));
        caps::set_color_override(None);
    }

    #[test]
    fn threshold_palette_progression() {
        assert_eq!(threshold_color(0.10), Color::Green);
        assert_eq!(threshold_color(0.80), Color::Yellow);
        assert_eq!(threshold_color(0.95), Color::Red);
    }

    #[test]
    fn render_fullscreen_inline_when_not_a_tty() {
        // In the test harness stdout is not a terminal, so this takes the inline
        // path and must succeed without emitting alt-screen control codes that
        // would corrupt captured output.
        let data = populated();
        assert!(render_fullscreen(&data).is_ok());
    }
    ...
    fn extreme_values_do_not_panic() {
        let data = DashboardData {
            target_gb: 1.0e9,
            target_cap_gb: 0.0, // zero cap -> fraction clamps to 0, no div-by-zero blowup
            history: vec![f64::MAX, 0.0, -1.0],
            policies: vec![("totally-unknown-tag".to_string(), "x".repeat(500))],
            ..Default::default()
        };
        let _ = render(&data);
    }

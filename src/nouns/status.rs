...
impl StatusShowVerb {
    fn execute(&self) -> anyhow::Result<()> {
        ...
        let dirty_word = if git.dirty_files.is_empty() && git.untracked_files.is_empty() { "clean" } else { "dirty" };
...
impl VerbCommand for StatusShowVerb {
    ...
    fn about(&self) -> &'static str { "Show full CI/CD status" }
    fn run(&self, _args: &VerbArgs) -> clap_noun_verb::error::Result<()> {
        self.execute().map_err(|e| clap_noun_verb::error::NounVerbError::execution_error(e.to_string()))
    }
}

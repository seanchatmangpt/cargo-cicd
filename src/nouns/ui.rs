...
mod tests {
    ...
    // The color/unicode overrides are process-wide atomics. Tests in this module
    // that force them must not run concurrently with one another, or one test's
    // override clobbers the other's. Serialize them behind a mutex and reset both
    // overrides to auto on drop (even on panic) — mirroring the pattern used by
    // the `crate::ui::*` component modules.
    static CAPS_LOCK: Mutex<()> = Mutex::new(());

    struct CapsGuard(#[allow(dead_code)] MutexGuard<'static, ()>);

    impl CapsGuard {
        fn acquire(color: Option<bool>, unicode: Option<bool>) -> Self {
            let g = CAPS_LOCK.lock().unwrap_or_else(|p| p.into_inner());
            caps::set_color_override(color);
            caps::set_unicode_override(unicode);
            CapsGuard(g)
        }
    }
    ...
    #[test]
    fn demo_renders_every_section_in_plain_mode() {
        let _g = CapsGuard::acquire(Some(false), Some(false));

        let out = render_demo();
        let plain = strip_ansi(&out);

        // Plain mode must not leak ANSI escapes.
        assert!(!plain.contains('\u{1b}'));

        // Each labeled section header is present.
        for section in [
            "Theme roles",
            "Status badges",
            "Table",
        ...
        // A representative sample from each component family survives rendering.
        assert!(plain.contains("PASS"), "badge label missing");
        assert!(plain.contains("Release readiness"), "panel title missing");
        assert!(plain.contains("sparkline"), "chart label missing");
        assert!(plain.contains("complete"), "progress steps summary missing");
        assert!(plain.contains("error"), "diagnostic severity missing");
        assert!(
            plain.contains("crates.io/crates/cargo-cicd"),

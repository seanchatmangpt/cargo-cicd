...
fn diff_baseline_logic(repo_dir: &Path, baseline: Option<String>) -> Result<()> {
    let baseline_path = baseline.ok_or_else(|| {
        clap_noun_verb::error::NounVerbError::execution_error("--diff requires --baseline <path>".to_string())
    })?;
    ...
    let mut new_counterexamples: Vec<String> = current_set.difference(&baseline_set).cloned().collect();
    new_counterexamples.sort();
    let mut resolved_counterexamples: Vec<String> = baseline_set.difference(&current_set).cloned().collect();
...
mod tests {
    ...
    fn doctor_diff_detects_new_counterexample() {
        ...
        fs::write(src_dir.join("fake_test.rs"), "fn foo() { assert!(true); }\n").unwrap();

...
fn fixture_target_over_limit_has_target_dir() {
    ...
    assert!(
        placeholder.exists(),
        "over-limit fixture must have target/debug/placeholder.bin"
    );
...
fn fixture_corrupted_cicd_toml_is_invalid_toml() {
    ...
    assert!(
        parsed.is_err(),
        "corrupted_cicd_toml fixture must fail TOML parse"
    );
}
...
fn fixture_stale_cicd_toml_claims_clean_but_is_dirty() {
    ...
    let content = std::fs::read_to_string(ws.root.join("cicd.toml")).unwrap();
    assert!(
        content.contains("dirty = false"),
        "stale cicd.toml must claim dirty=false"
    );
    assert!(
        ws.root.join("untracked.txt").exists(),
        "stale fixture workspace must actually be dirty"
    );
}
...
fn fixture_changed_trybuild_has_ui_dir() {
    ...
    assert!(
        changed.exists(),
        "trybuild fixture must have changed_fixture.rs"
    );
    ...
    for i in 0..10 {
        ...
        assert!(
            existing.exists(),
            "trybuild fixture must have existing_{i:02}.rs"
        );
    }
}

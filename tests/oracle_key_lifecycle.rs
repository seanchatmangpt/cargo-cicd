...
use cargo_cicd::receipt_validation::{
    parse_receipt_json, receipt_has_required_fields, validate_receipt_file,
...
fn load_oracle_key_returns_none_when_env_unset() {
    ...
    assert!(result.is_none(), "expected None when CICD_ORACLE_KEY_B64 is not set");
}
...
fn load_oracle_key_returns_key_when_env_set() {
    ...
    let result = load_oracle_key_from_env();
    assert!(result.is_some(), "expected Some when CICD_ORACLE_KEY_B64 is set");
...
fn key_is_valid_false_for_expired_key() {
    ...
    assert!(!key_is_valid(&key), "key with past valid_until must be invalid");
}
...
fn oracle_key_trace_attributes_non_empty() {
    ...
    assert!(has_fingerprint, "attributes must include oracle:fingerprint");
...
fn compute_fingerprint_non_empty_for_any_key() {
    for input in &["aGVsbG8=", "dGVzdA==", "AAAA", "", "not_base64_!!"] {
        ...
        assert!(!fp.is_empty(), "fingerprint must be non-empty for input {:?}", input);
    }
}
...
fn parse_receipt_json_valid_json() {
    let json = r#"{"verdict":"Accept","timestamp":"2026-06-17T10:00:00Z","oracle_id":"wasm4pm/v26.5.29"}"#;
